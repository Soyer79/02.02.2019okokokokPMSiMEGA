...
#define MyDB "meteo"
#define INSERT "insert into meteo.mega(tempZew,wilgZew,tempSlonce,tempGrunt,cisnienie) values(%d,%d,%d,%d,%d)"
...
void storeData(int temperatura_dht_zew, int wilgotnosc_dht_zew, int temp_slonce, int temp_grunt, int cisnienie_bmp_strych)
{
    if (conn.connect(MyServer, 3306, MyUSER, MyPASS)) {
        char query[128];
        MySQL_Cursor *cursor = new MySQL_Cursor(&conn);
        sprintf(query,INSERT,temperatura_dht_zew,wilgotnosc_dht_zew,temp_slonce,temp_grunt,cisnienie_bmp_strych);
...
void menu_glowne(){
 ...
 if(czujniki.available()){
  ...
  PM_maxmin_reset();
  storeData(temperatura_dht_zew, wilgotnosc_dht_zew, temp_slonce, temp_grunt, cisnienie_bmp_strych);
